...
export default function PublicFormView({ formId, onBack }: Props) {
  const form: CustomForm | undefined = dataService.getFormById(formId);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-slate-50">
        <div className="bg-white p-8 rounded-3xl border border-slate-200 text-center max-w-md shadow-sm">
          ...
          <p className="text-xs text-slate-500 mt-1 mb-6">The requested registration form does not exist or has been archived.</p>
          <button
            onClick={onBack}
        ...
        {submitted ? (
          <div className="bg-white rounded-3xl p-8 sm:p-12 border border-slate-100 shadow-sm text-center space-y-4">
            ...
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              Your information has been successfully received and recorded in our school administrative dashboard.
            </p>
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 text-xs font-mono text-slate-700 inline-block">
              Submission ID: <strong className="text-blue-900">{submissionId}</strong>
            </div>
            <div className="pt-4">
              <button
                onClick={onBack}
